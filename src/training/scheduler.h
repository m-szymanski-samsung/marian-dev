...
private:
  ...
  LoggingContainer update_stats_logger_;
  ...
  struct CostStatsDisplay : public LoggedStat {
    Ptr<TrainingState> state_;
    std::string lossType_;
    bool dispLabelCounts_;
    CostStatsDisplay(Ptr<TrainingState> state, Ptr<Options> options) : LoggedStat("",""), state_(state) {
    ...
    std::string format_value() const override { return formatLoss(lossType_, dispLabelCounts_, 0/*ignore batchLabels*/, state_);};
    void reset() override {state_->since_last_display.reset();}
    static std::string formatLoss(std::string lossType,
                          bool dispLabelCounts,
                          size_t batchLabels,
                          Ptr<TrainingState> state) {
      const auto &since_disp = state->since_last_display;
      std::stringstream ss;
      ss << "Cost ";
...
public:
  ...
  Scheduler(Ptr<Options> options, Ptr<TrainingState> state)
    ...
    update_stats_logger_.add_variable("Ep.", state_->epochs);
    update_stats_logger_.add_variable("Up.", state_->batches);
    update_stats_logger_.add_lambda("Sen.", [&](){ return utils::withCommas(state_->samplesEpoch);});
    update_stats_logger_.add_object(New<CostStatsDisplay>(state_,options_));
    update_stats_logger_.add_lambda("Time", "{:.2f}", [&](){return timer_.elapsed();}, "s", [&](){timer_.start();});
    update_stats_logger_.add_lambda("", "{:.2f}", [&](){return state_->since_last_display.words / timer_.elapsed();}, " words/s");
    if(options_->get<bool>("lr-report")) update_stats_logger_.add_variable("L.r.", "{:.4e}", state_->eta);
  }
  ...
  void update(StaticLoss rationalLoss,
    ...
    state_->since_last_display.update(rationalLoss, batchSize, batchLabels);
    ...
    if(state_->enteredNewPeriodOf(options_->get<std::string>("disp-freq")) ||
      ...
      if(mpi) {
        state_->since_last_display.costSum /= mpi->numMPIProcesses(); // undo the extra scaling
        mpi->allReduce(&state_->since_last_display.costSum, &state_->since_last_display.costSum, 1, MPI_FLOAT, MPI_SUM);
      }

      if(mpi && mpi->myMPIRank() != 0) {
        // skip the report on alternate worker processes
      } else {
        update_stats_logger_.log_info();
      }

      update_stats_logger_.reset_all();
    ...
    if((!mpi || mpi->myMPIRank() == 0) && getenv("PHILLY_JOB_ID")
      ...
      printf("PROGRESS: %.2f%%\nEVALERR: %.7f%%\n",
          ...
          state_->since_last_display.costSum / state_->since_last_display.costCount / (mpi ? mpi->numMPIProcesses() : 1));
      fflush(stdout);
  ...
  void load(const std::string& name) {
    ...
    if(options_->get<bool>("no-restore-corpus")) {
      ...
      state_->since_last_display.reset();
    }
